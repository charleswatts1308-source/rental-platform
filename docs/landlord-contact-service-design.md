...
7. MariaDB strict-mode migration fixes — HUK's MariaDB 10.3.30 in strict mode rejects `TIMESTAMP NOT NULL` columns without explicit defaults. The `cases` table migration trips this rule on `opened_at`; other LLCS migrations with similar columns likely do too (suspects: `case_events`, `case_messages`, `message_attachments`, `reply_tokens` — wherever timestamps are not nullable and not `useCurrent()`). Local dev DB is permissive enough to let these pass. Tactical unblock: `'strict' => false` on the `mariadb` connection in `config/database.php` (to be applied if staging deploy still requires it after this doc commit). Proper fix: audit every LLCS migration's TIMESTAMP NOT NULL columns and add either `->useCurrent()` (for created-on-insert timestamps) or `->nullable()` (for optional event timestamps). Re-enable strict mode once done.
8. PHP CLI extension config mismatch on HUK Linux Plesk — HUK's PHP 8.4 alt-php build has a broken CLI extension load order: `pdo_mysql` fails to load because base `pdo` isn't loaded first, plus a malformed `extension=` line producing a `.so.so` suffix. Web FastCGI PHP is fine; only CLI is affected. Workaround in use: every artisan invocation prefixed with `-c /var/www/vhosts/system/ukrenters.rent/etc/php.ini` to force CLI to use the per-site config that works for web. Plesk's scheduled task for `php artisan schedule:run` will need the same `-c` flag included in the cron command line until HUK resolves the underlying packaging issue. HUK ticket raised 2026-05-16. Once resolved, drop the `-c` workaround from the cron entry.
9. Test suite — Pwned Passwords failures in auth tests — Three auth tests (`PasswordResetTest > password can be reset with valid token`, `PasswordUpdateTest > password can be updated`, `RegistrationTest > new users can register`) fail because they use the literal string `"password"` as test input, which Laravel's HIBP-backed `uncompromised()` validator correctly rejects. Fix: change test passwords to `Str::random(20)` or another non-pwned constant. Trivial change; not blocking.
10. Local dev environment — GD extension on desktop — The desktop PHP install doesn't have GD enabled, causing three Phase 6 photo-attachment tests to fail locally with `LogicException: GD extension is not installed`. HUK's PHP has GD. Not a code issue. Fix: enable `extension=gd` in desktop `php.ini`. Pure local-environment housekeeping.
11. Stray nested folder `database/migrations/database/migrations/` — A nested duplicate `database/migrations/` directory exists from November 2025, unrelated to the LLCS work. Laravel only reads the top-level `database/migrations/` so it doesn't affect runtime. Investigate origin and tidy up post-deploy.

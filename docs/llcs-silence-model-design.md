...
- **#14 / #15 / #21-tail** — "Next escalation" shown on `on_hold`; stale `hold_until` after release; "Next escalation" reappearing on the abandoned/exhausted card. One shared **state-aware-display predicate**; Surface C's next-sweep projection must use the same predicate or it reinvents this bug family. Fix together. The predicate's terminal rule is fixed by the #21 ruling: **terminal states show no "next escalation" and no live actions.**
- **#21 — RULED 21 Jun: exhausted = dead = terminal.** An `escalation_exhausted` case has proven landlord non-engagement in any meaningful way — job done. At platform startup it is dead and goes no further. The dead card carries **no live controls**: the "Abandon this case" action is absent (abandon is a transition, and a terminal case has nowhere to transition to — the collision dissolves rather than being arbitrated), and the cosmetic stance dropdown is not offered live. **No future-provision and no placeholder note** — the volume of exhausted cases is unknown (a minority, but unquantified) and no suitable post-exhaustion action is known, so nothing concrete enough exists to provision or even note; provisioning for an unpredictable future is the soft form of the speculative build and is cut entirely. If a real post-exhaustion need ever arises it is designed then, from the actual need. **This ruling supersedes both conflicting #21 blocks in the snag file** (the "open design question" block and the prematurely-written "RULING" block) and **unblocks the D14 close-out reference** — D14 may now be doc-updated to point at this ruling.
...
- ~~#21 abandon-collision.~~ **Ruled: exhausted = dead, record-only, no future-provision.** (#21.)

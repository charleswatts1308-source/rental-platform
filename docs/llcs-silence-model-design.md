...
- **Exhausted cases: see the #21 ruling below, which governs.** *(This bullet previously read "record-only, no live controls, no Abandon action". That was the withdrawn draft framing and is wrong — `escalation_exhausted` is not terminal, it has four outgoing edges, and the tenant keeps reply/resolve/abandon. What Option C actually removed is the cosmetic stance dropdown alone. Corrected 19 Sep 2026; it had been contradicting the ruling nine lines below it since 21 Jun.)* Surface C is read-only in any case, so nothing built on this bullet.
